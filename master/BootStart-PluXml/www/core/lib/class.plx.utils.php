...
class plxUtils {
	...
	*/
	...
	public static function printSelect($name, $array, $selected='', $readonly=false, $class='', $id=true) {
		...
		$id = ($id?' id="id_'.$name.'"':'');
		if($readonly)
			echo '<select'.$id.' name="'.$name.'" disabled="disabled" class="'.$class.'">'."\n";
		else
	 ...
	 **/
	public static function testWrite($file, $format="<div class=\"alert #alert\">#symbol #message</div>") {

		if(is_writable($file)) {
			$output = str_replace('#alert', 'alert-info', $format);
			$output = str_replace('#symbol', '&#10004;', $output);
			$output = str_replace('#message', sprintf(L_WRITE_ACCESS, $file), $output);
			echo $output;
		} else {
			$output = str_replace('#alert', 'alert-error', $format);
			$output = str_replace('#symbol', '&#10007;', $output);
	 ...
	 * @author	Stephane F
	 **/
	public static function testModRewrite($io=true, $format="<div class=\"alert #alert\">#symbol #message</div>") {
	 ...
	 * @author	Stephane F
	 **/
	public static function testMail($io=true, $format="<div class=\"alert #alert\">#symbol #message</div>") {
	 ...
	 * @author	Stephane F
	 **/
	public static function testLibGD($format="<div class=\"alert #alert\">#symbol #message</div>") {
	...
	public static function showMsg($msg, $class='') {
		...
		if($class=='') echo '<div class="msg alert alert-info"><button type="button" class="close" data-dismiss="alert">&times;</button>'.$msg.'</div>';
		else echo '<div class="msg alert alert-'.$class.'"><button type="button" class="close" data-dismiss="alert">&times;</button>'.$msg.'</div>';
	}
