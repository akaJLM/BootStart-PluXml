...
<div class="row-fluid">
  <div class="span12 widget">
    <div class="widget-title"><span class="icon"><i class="icon-comments icon-grey icon-shadowed"></i></span>
      <h5><?php echo L_CREATE_NEW_COMMENT ?></h5>
      <?php if(!empty($_GET['a'])) : ?>
    ...
    <div class="widget-content"> <span class="label pull-right" style="margin-right:7px"><?php echo L_COMMENTS_ARTICLE_SCOPE ?> &laquo;<?php echo plxUtils::strCheck($aArt['title']); ?>&raquo;</span>
      <div class="clearfix" style="margin-bottom: 5px;"></div>
      <?php eval($plxAdmin->plxPlugins->callHook('AdminCommentNewTop')) # Hook Plugins ?>
      <form class="form form-horizontal" action="comment_new.php?<?php echo plxUtils::strCheck($get) ?>" method="post" id="form_comment">
        ...
        <div class="control-group">
          <label class="control-label" for="id_content"><?php echo L_CREATE_NEW_COMMENT ?></label>
          <div class="controls">
        ...
        <div class="control-group">
          ...
          <div class="controls">
            <div class="btn-group">
              <input class="btn btn-responsive new" type="submit" name="create" value="<?php echo L_COMMENT_SAVE_BUTTON ?>"/>
            </div>
          </div>
        </div>
      </form>
...
<div class="row-fluid">
  <div class="span12 widget">
    <div class="widget-title"><span class="icon"><i class="icon-comments icon-grey icon-shadowed"></i></span>
      <h5><?php echo L_ARTICLE_COMMENTS_LIST ?></h5>
    </div>
    <div class="widget-content">
      ...
      <?php 
            ...
            ?>
      <span class="badge badge-info <?php echo 'pull-'.$sideInverse.''; ?>" style="<?php echo 'margin-'.$sideInverse.''; ?>: 47.66%; margin-top: 6.1%;"> <?php echo plxDate::formatDate($plxAdmin->plxRecord_coms->f('date'), '#hour:#minute'); ?> </span>
      <div class="btn-group btn-group-vertical <?php echo 'pull-'.$side; ?>"> <a class="btn btn-mini" rel="tooltip" data-toggle="tooltip" data-placement="<?php echo $sideInverse; ?>" data-original-title="<?php echo L_COMMENT_EDIT_TITLE ?>" href="comment.php<?php echo (!empty($_GET['a']))?'?c='.$comId.'&amp;a='.$_GET['a']:'?c='.$comId; ?>"><i class="icon-edit"></i></a> <a class="btn btn-mini" rel="tooltip" data-toggle="tooltip" data-placement="<?php echo $sideInverse; ?>" data-original-title="<?php echo L_COMMENT_ANSWER_TITLE ?>" href="javascript:answerCom('content','<?php echo $plxAdmin->plxRecord_coms->f('numero'); ?>','<?php echo plxUtils::strCheck($plxAdmin->plxRecord_coms->f('author')) ?>');"><i class="icon-reply"></i></a> </div>
      <blockquote style="max-width: 40%;" class="<?php echo 'pull-'.$side; ?> type-<?php echo $plxAdmin->plxRecord_coms->f('type') ?>"> <?php echo nl2br($plxAdmin->plxRecord_coms->f('content')); ?>
        <hr>
        <span class="label <?php echo 'pull-'.$side; ?> comment<?php echo ((isset($_GET['c']) AND $_GET['c']==$comId?' current':'')) ?>" id="c<?php echo $plxAdmin->plxRecord_coms->f('numero'); ?>"> <?php echo L_COMMENT_WRITTEN_BY ?>&nbsp;<span class="label label-info"><?php echo $plxAdmin->plxRecord_coms->f('author'); ?></span> @ <?php echo plxDate::formatDate($plxAdmin->plxRecord_coms->f('date'), '#day #num_day #month #num_year(4)'); ?> </span> </blockquote>
